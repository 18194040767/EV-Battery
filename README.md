...
- 不要提交运行日志、构建产物、依赖目录、缓存目录和本地生成文件。
- 前端依赖通过 `npm install` 恢复，后端依赖通过 Maven 恢复。
- 数据库密码、API Key、私钥等敏感信息不要写入仓库，请使用环境变量或本地配置。
- 如需重新生成前端产物，请在本地运行 `npm run build`，生成的 `dist/` 不需要提交。
